...
console.log(props.items)
  ...
  function do_vote(para){
    ...
    // axios.post('http://localhost:5000/org', {
      ...
      props.instance.methods.do_vote(para).send({from: props.wallet_address}).then((data)=>{console.log(data);
  ...
  //props.connect.methods.get_Result().call({from:props.address[0]}).then(data=>console.log(data))
    ...
    <li className="list-group-item"> <p className='fw-bold'> PartName : {item.DOB}</p></li>
    <li className="list-group-item"><p className='fw-bold'> PartName : {item.DOB}</p></li>
